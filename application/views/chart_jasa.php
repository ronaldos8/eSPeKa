        ...
        <div class="col-md-12">
          ...
          <div class="box box-primary">
            ...
            <select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" class="form-control">
                ...
                <option value="<?php echo site_url('home/index/jasa') ?>" selected>Berdasarkan Jasa Pengiriman</option>
                <option value="<?php echo site_url('home/index/kriteria') ?>">Berdasarkan Kriteria</option>
            </select>
    ...
    $(function () {
      ...
      var bar = new Morris.Bar({
        ...
        data: [
          <?php
            if ($jasa != NULL) {
              ...
              foreach($jasa as $key => $value){
                if ($c2 == $c-1) {
          ?>
                  {y: '<?php echo $jasa[$value->ID_jasa]->nama_jasa; ?>', <?php $x = 1; foreach($data_kriteria as $value2){
                      if($x == 1){
                        if (isset($kriteria[$value->ID_jasa][$value2->ID_kriteria])) {
                          echo $value2->ID_kriteria .": " .$kriteria[$value->ID_jasa][$value2->ID_kriteria]*100;
                        }else {
                          echo $value2->ID_kriteria .": 0";
                        }
          ...
          ?>
                  {y: '<?php echo $jasa[$value->ID_jasa]->nama_jasa; ?>', <?php $y = 1; foreach($data_kriteria as $value2){
                      if($y == 1){
                        if (isset($kriteria[$value->ID_jasa][$value2->ID_kriteria])) {
                          echo $value2->ID_kriteria .": " .$kriteria[$value->ID_jasa][$value2->ID_kriteria]*100;
                        }else {
        ...
        ykeys: [<?php
              $c = 1;
              foreach($data_kriteria as $value){
                if ($c == 1) {
                  echo "'" .$value->ID_kriteria ."'";
                }else {
                  echo ",'" .$value->ID_kriteria ."'";
                }
                $c++;
